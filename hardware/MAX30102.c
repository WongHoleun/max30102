...
// 全局变量
volatile uint32_t max30102_led1_buffer[BUFFER_SIZE_MAX3010x] = {0};		// LED1 传感器数据
volatile uint32_t max30102_led2_buffer[BUFFER_SIZE_MAX3010x] = {0};	// LED2 传感器数据
volatile uint16_t max30102_buffer_head = 0;		// 缓存器头
volatile uint16_t max30102_buffer_tail = 0;		// 缓存器尾
volatile uint16_t max30102_collected_samples = 0; // 采集的样本
uint8_t heart_rate_max30102 = 0;					// 心率值
int8_t hr_valid_max30102 = 0;					// 心率有效
float spo2_value_max30102 = 0;						// 血氧值
int8_t spo2_valid_max30102 = 0;					// 血氧值有效
float ratio, correl;

// 状态机初始化
Max3010xState max30102_state;
...
{
	uint8_t tmp;
	if (MAX30102_OK != Max30102_ReadReg(Register, &tmp)) // 先获取对应寄存器的值，并暂存到tmp，并判断IIC是否正常读取
	{
		return MAX30102_ERROR;
	}
	...
	if (MAX30102_OK != Max30102_WriteReg(Register, tmp)) // 将tmp的值写入寄存器，并判断IIC是否正常写入
	{
		return MAX30102_ERROR;
	}
...
void Max30102_InterruptCallback(void)
{
	...
	if (Status & (1 << INT_A_FULL_BIT)) // 判断是否产生数据将满中断
	{
		for (uint8_t i = 0; i < MAX3010x_FIFO_ALMOST_FULL_SAMPLES; i++)
		{
			...
			max30102_buffer_head = (max30102_buffer_head + 1) % BUFFER_SIZE_MAX3010x; // 缓冲器头指针加一,通过求余运算符进行循环（求余%：如果a<b的话，a%b的商为0,余数就是a）
			max30102_collected_samples++;							 // 继续采集样本
		}
	}
...
{
	return heart_rate_max30102;
}
/*血氧获取*/
float Max30102_GetSpO2Value(void)
{
	return spo2_value_max30102;
}
...
void Max30102_Task(void)
{
	...
	switch (max30102_state)
	{
		case MAX3010x_STATE_BEGIN: // 现态：准备采样状态
		{
			...
			/*动作*/
			max30102_collected_samples = 0; // 采集样本为0
			max30102_buffer_tail = max30102_buffer_head;		  // 缓存区头尾相连
			Max30102_Led1PulseAmplitude(MAX3010x_RED_LED_CURRENT_HIGH); // 红LED~7mA
			Max30102_Led2PulseAmplitude(MAX3010x_IR_LED_CURRENT_HIGH);	// 红外~7mA

			/*次态*/
			max30102_state = MAX3010x_STATE_CALIBRATE; // 检测到手指接近 转换为校准态
			break;
		...
		case MAX3010x_STATE_CALIBRATE: // 现态：物体接触到传感器，采集过去5秒样本将有效数据填充缓冲区
		{
			...
			/*条件*/
			if (max30102_collected_samples > BUFFER_SIZE) // 采集的样本数超出缓冲区空间
			{
				/*次态*/
				max30102_state = MAX3010x_STATE_CALCULATE; // 采集到一定数量的样本 转换为计算态
			}
			break;
...
//					max30102_buffer_tail);
			...
			max30102_buffer_tail = (max30102_buffer_tail + FS) % BUFFER_SIZE_MAX3010x; // 缓冲区尾
			max30102_collected_samples = 0;						  // 收集到的样本数归零

			/*次态*/
			max30102_state = MAX3010x_STATE_COLLECT_NEXT_PORTION; // 采集态：继续采集新的样本
			break;
		...
		{
			/*条件 2*/
			if (max30102_collected_samples > FS) // 已有样本超出每秒平均样本数
			{
				/*次态*/
				max30102_state = MAX3010x_STATE_CALCULATE; // 对采集的样本进行算法处理
			}
			flag = true;
			break;
...
MAX30102_STATUS Max30102_Init(I2C_HandleTypeDef *i2c)
{
	...
	/*软件复位*/
	if (MAX30102_OK != Max30102_Reset()) // 复位 MAX30102
	{
		return MAX30102_ERROR;
	}
	/*读寄存器*/
	...
	/*FIFO采样平均*/
	if (MAX30102_OK != Max30102_FifoSampleAveraging(FIFO_SMP_AVE_1)) // 每个 FIFO 样本的平均样本数为4
	{
		return MAX30102_ERROR;
	}
	/*FIFO循环使能*/
	if (MAX30102_OK != Max30102_FifoRolloverEnable(0)) // 设置为0，FIFO不会更新，直到读取 FIFO_DATA 或更改 WRITE/READ 指针位置。
	{
		return MAX30102_ERROR;
	}
	/*FIFO满值设置*/
	if (MAX30102_OK != Max30102_FifoAlmostFullValue(MAX3010x_FIFO_ALMOST_FULL_SAMPLES)) //  未读样本X个时发出中断。
	{
		return MAX30102_ERROR;
	}
	/*模式设置*/
	if (MAX30102_OK != Max30102_SetMode(MODE_SPO2_MODE)) // 血氧模式
	{
		return MAX30102_ERROR;
	}
	/*ADC范围*/
	if (MAX30102_OK != Max30102_SpO2AdcRange(SPO2_ADC_RGE_8192)) // ADC满量程4096nA
	{
		return MAX30102_ERROR;
	}
	/*采样率*/
	if (MAX30102_OK != Max30102_SpO2SampleRate(SPO2_SAMPLE_RATE_1000)) // 采样率
	{
		return MAX30102_ERROR;
	}
	/*LED脉冲宽度*/
	if (MAX30102_OK != Max30102_SpO2LedPulseWidth(SPO2_PULSE_WIDTH_411)) // 脉冲宽度411us 分辨率18bits
	{
		return MAX30102_ERROR;
	}
	/*红色LED脉冲幅度*/
	if (MAX30102_OK != Max30102_Led1PulseAmplitude(MAX3010x_RED_LED_CURRENT_LOW)) // 红色LED脉冲幅值0mA
//	if (MAX30102_OK != Max30102_Led1PulseAmplitude(MAX3010x_RED_LED_CURRENT_HIGH)) // 红色LED脉冲幅值0mA
	{
		return MAX30102_ERROR;
	}
	/*红外LED脉冲幅度*/
	if (MAX30102_OK != Max30102_Led2PulseAmplitude(MAX3010x_IR_LED_CURRENT_LOW)) // 红外LED脉冲幅值0.2mA
//	if (MAX30102_OK != Max30102_Led2PulseAmplitude(MAX3010x_IR_LED_CURRENT_HIGH)) // 红外LED脉冲幅值0.2mA
	{
		return MAX30102_ERROR;
	}
	/*FIFO 中断使能 */
	...
	max30102_state = MAX3010x_STATE_BEGIN; // 状态机初始状态
